//finding and storing the form element using its ID so action can be taken once its submitted
const form = document.getElementById("moodForm");
//get and store the section where results like messages and images will be displayed
const resultSection = document.getElementById("result");
//locate then store the area where the mood messages will be shown
const message = document.getElementById("message");
//get and store the element where the users written note will appear
const userNote = document.getElementById("userNote");
//find and store the image element that will visually reflect the selected mood
const moodImage = document.getElementById("moodImage");

//adding an event listener for when the form submits, so code can run a function to process the input
form.addEventListener("submit", function (event) {
  event.preventDefault(); //stops the page from refreshing or reloading on submit
  //grabbing the form data to store it
  const formData = new FormData(form);
  //turning the form data into an object so I can work with it
  const data = Object.fromEntries(formData.entries());

  //pulled out the mood and note from the form object and stored into variables
